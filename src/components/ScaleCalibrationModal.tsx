...
export function ScaleCalibrationModal(props: { media: Media; url: string; onClose: () => void }) {
  ...
  async function save() {
    ...
    // We need to account for the fact that the image might be scaled in the UI
    ...
    const pxPerMm = naturalDistPx / parseFloat(mm);

    await db.media.update(props.media.id, { pxPerMm, scalePresent: true });
    props.onClose();
  }
  ...
  return (
    <Modal onClose={props.onClose} title="Calibrate Digital Scale">
      <div className="grid gap-4">
        ...
        <div className="relative cursor-crosshair border-2 border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden bg-black flex items-center justify-center">
          <img 
            ref={imgRef}
            src={props.url} 
